...
def get_M_for_u(h_):
    # integrate
    res1 = sp.integrate(
        x**alpha * p * w0lh_j_p**2 + q * w0lh_j**2, (x, h * j, h * (j + 1))
    ).simplify()
    res2 = sp.integrate(
        x**alpha * p * w0rh_j_p**2 + q * w0rh_j**2, (x, h * (j + 1), h * (j + 2))
    ).simplify()
    M_00_diag_inner = (res1 + res2).simplify()

    M_00_diag_first = (
        sp.integrate(x**alpha * p * w0rh_j_p**2 + q * w0rh_j**2, (x, 0, h))
        .subs({j: -1})
        .simplify()
    )
    M_00_subdiag = sp.integrate(
        x**alpha * p * w0rh_j_p * w0lh_j_p_next + q * w0rh_j * w0lh_j_next,
        (x, h * (j + 1), h * (j + 2)),
    )
